...
class Petolio_Model_PoOnlineMapper {
	protected $_dbTable;
	...
	public function find($id, Petolio_Model_PoOnline $cls) {
	...
	public function fetchAll() {
		...
		foreach($this->getDbTable()->fetchAll() as $row) {
			$cls = new Petolio_Model_PoOnline();
			$cls->setId($row->id)
				->setFromId($row->from_id)
				->setToId($row->to_id)
				->setMessage($row->message)
	...
	public function findOneByField($field, $value, Petolio_Model_PoOnline $cls) {
	...
	public function getDbTable() {
		if(is_null($this->_dbTable))
			$this->setDbTable('Petolio_Model_DbTable_PoOnline');

		return $this->_dbTable;
	}

	public function save(Petolio_Model_PoOnline $cls, $ignoreEmpty = true, $escapeValues = false) {
		$data = $cls->toArray();
		if($ignoreEmpty)
