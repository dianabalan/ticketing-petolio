 ...
 * File:        Thumbnail.php
 *
 * @copyright 2010 Roman Krivosheyev, Personal.
 * @author Roman Krivosheyev <web.volga@gmail.com>
 * @author Roman Krivosheyev
 * @package Thumbnail
 * @version 2.5.0
 *
 *
 ...
 *     'mark'  => 0,
 * );
 * Petolio_Service_Image::output($filename, 'data.jpg', $options);
 *
 *
 */
...
class Petolio_Service_Image
{
     ...
     * @access public
     * @static
     * @see    Petolio_Service_Image::imageCreateFromFile(), Petolio_Service_Image::imageCreateFromString()
     */
    private static function imageCreate($input)
    {
        if ( is_file($input) ) {
            return Petolio_Service_Image::imageCreateFromFile($input);
        } else if ( is_string($input) ) {
            return Petolio_Service_Image::imageCreateFromString($input);
        } else {
            return $input;
        }
    }
    ...
    public static function output($input, $output=null, $options=array())
    {
        ...
        $renderImage = Petolio_Service_Image::render($input, $options);
        if ( ! $renderImage ) {
     ...
     * @return boolean TRUE on success or FALSE on failure.
     * @access public
     * @see    Petolio_Service_Image::output()
     */
    private static function render($input, $options=array())
    {
        // Create the source image
        $sourceImage = Petolio_Service_Image::imageCreate($input);
        ...
        if ( !isset($options['height']) || strcasecmp("0", $options['height']) == 0 ) {
        	$options['height'] = ( $sourceHeight * intval($options['width']) ) / $sourceWidth;
        }
        ...
        if ( $options['method'] == THUMBNAIL_METHOD_CROP ) {
            ...
            $Y = Petolio_Service_Image::_coord($options['valign'], $sourceHeight, $H);
            $X = Petolio_Service_Image::_coord($options['halign'], $sourceWidth,  $W);
        } else {
            $X = 0;
            $Y = 0;
