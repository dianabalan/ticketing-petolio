...
class Petolio_Model_PoStarRatingsMapper {
	...
	public function find($id, Petolio_Model_PoStarRatings $cls) {
	...
	public function fetchAll() {
		...
		foreach($this->getDbTable()->fetchAll() as $row) {
			$cls = new Petolio_Model_PoStarRatings();
			$cls->setId($row->id)
				->setUserId($row->user_id)
				->setRating($row->rating)
				->setScope($row->scope)
	...
	public function findOneByField($field, $value, Petolio_Model_PoStarRatings $cls) {
	...
	public function getDbTable() {
		if(is_null($this->_dbTable))
			$this->setDbTable('Petolio_Model_DbTable_PoStarRatings');

		return $this->_dbTable;
	}

	public function save(Petolio_Model_PoStarRatings $cls, $ignoreEmpty = true, $escapeValues = false) {
		$data = $cls->toArray();
		if($ignoreEmpty)
	...
	public function getEntityRating($scope, $entity_id) {
		$table = $this->getDbTable();
		return $table->getEntityRating($scope, $entity_id);
	}
}