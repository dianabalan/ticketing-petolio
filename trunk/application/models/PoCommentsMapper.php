...
class Petolio_Model_PoCommentsMapper {
	protected $_dbTable;
	...
	public function find($id, Petolio_Model_PoComments $cls) {
	...
	public function fetchAll() {
		...
		foreach($this->getDbTable()->fetchAll() as $row) {
			$cls = new Petolio_Model_PoComments();
			$cls->setId($row->id)
				->setUserId($row->user_id)
				->setScope($row->scope)
				->setEntityId($row->entity_id)
	...
	public function findOneByField($field, $value, Petolio_Model_PoComments $cls) {
	...
	public function getDbTable() {
		if(is_null($this->_dbTable))
			$this->setDbTable('Petolio_Model_DbTable_PoComments');

		return $this->_dbTable;
	}

	public function save(Petolio_Model_PoComments $cls, $ignoreEmpty = true, $escapeValues = false) {
		$data = $cls->toArray();
		if($ignoreEmpty)
