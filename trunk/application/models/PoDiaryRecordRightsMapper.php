...
class Petolio_Model_PoDiaryRecordRightsMapper {

    /**
     * $_dbTable - instance of Petolio_Model_DbTable_PoDiaryRecordRights
     *
     * @var Petolio_Model_DbTable_PoDiaryRecordRights
     */
    protected $_dbTable;
     ...
     * @param string $field
     * @param mixed $value
     * @param Petolio_Model_PoDiaryRecordRights $cls
     */
     ...
     *
     * @param new Petolio_Model_PoDiaryRecordRights $cls
     * @return array
     *
     */
    ...
    public function findByField($field, $value, $cls)
    {
            ...
            foreach ($rows as $row) {
                    $cls = new Petolio_Model_PoDiaryRecordRights();
                    $result[]=$cls;
		            $cls->setId($row->id)
						->setDiaryRecordId($row->diary_record_id)
						->setUserId($row->user_id);
     ...
     *
     * @param Petolio_Model_DbTable_PoDiaryRecordRights $dbTable
     ...
     *
     * @return Petolio_Model_DbTable_PoDiaryRecordRights
     */
    public function getDbTable()
    {
        if (null === $this->_dbTable) {
            $this->setDbTable('Petolio_Model_DbTable_PoDiaryRecordRights');
        }
    ...
    public function save(Petolio_Model_PoDiaryRecordRights $cls,$ignoreEmptyValuesOnUpdate=true, $escapeValues = false)
    {
       	$data = $cls->toArray();
    ...
    public function find($id, Petolio_Model_PoDiaryRecordRights $cls)
    {
    ...
    public function fetchAll()
    {
        ...
        foreach ($resultSet as $row) {
            $entry = new Petolio_Model_PoDiaryRecordRights();
            $entry->setId($row->id)
				  ->setDiaryRecordId($row->diary_record_id)
				  ->setUserId($row->user_id)
				  			  ->setMapper($this);
    ...
    /**
     * returns an array of PoUsers who have access to this diary record
     *
     * @param int $diary_record_id
     */
    public function findAllowedUsers($diary_record_id = -1) {
        $resultSet = $this->getDbTable()->findAllowedUsers($diary_record_id);
        $entries   = array();
