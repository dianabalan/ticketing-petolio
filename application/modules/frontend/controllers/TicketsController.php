...
class TicketsController extends Zend_Controller_Action
{
    ...
    public function manageTicketsAction()
    {
		...
		//$clients_tickets = $manager->fetchClientTickets($user_id);
        $page = $this->request->getParam("page");
        $page = $page ? $page : 0;
		$items_per_page = $this->cfg["products"]["pagination"]["itemsperpage"];
		$page = 1;

		$this->view->links = array('edit_status');
		$this->view->list = $manager->getSpTickets($user_id, $items_per_page, $page);

    }

    public function manageTicketEditAction() {

		$this->view->title = $this->translate->_("Edit Ticket Status");
		$form = new Petolio_Form_Ticket_TicketStatus();

		if(!$ticket_id = $this->request->getParam("ticket")) {
			return $this->_redirect('/tickets/manage-tickets');
		}
        ...
        $ticket = $manager->getSpTicket($user_id, $ticket_id);
        if (!$ticket) {
			return $this->_redirect('/tickets/manage-tickets');
        }
		...
		if(!$this->request->isPost()) {
			$form->setDefaults(array(
				'status' => $ticket['flgworkflow']
			));
		}
		else if($form->isValid($_POST)) {
			//echo 'valid form';
			$post = $this->request->getPost();
			$wf_manager = new Petolio_Model_Ticket_TicketsClientWfManager();
			$wf_manager->updateFlgworkflowTicketsClientWf($ticket_id, $post['status']);
			return $this->_redirect('/tickets/manage-tickets');
		}
