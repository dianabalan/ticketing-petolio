	...
	public function preDispatch() {
		...
		$view->auth = $auth;
       	$view->logoutUrl = $view->url(array(
       							'controller'=>'index',
              					'action'=>'logout'), 'editor', true);
		if(!($auth->hasIdentity() && $auth->getIdentity()->is_editor == 1)) {
        	if($auth->hasIdentity()) {
				...
				return $flux->logout($this->getFrontController()->getBaseUrl().'/editor');
        	}
	            ...
	            if($form->isValid($request->getPost())) {
					...
					if($result->isValid()) {
						...
						if($user->is_editor == 1) {
							...
							// forum login
							$flux = new Petolio_Service_FluxBB();
							$flux->login($user->id, $this->getFrontController()->getBaseUrl().'/editor');
						} else {
							$auth->clearIdentity();
							$this->msg->messages[] = $this->translate->_("Your account does not have editor priviledges.");
						}
					} else {
