	...
	public function errorAction() {
		...
		switch ($errors->type) {
			...
			case Zend_Controller_Plugin_ErrorHandler::EXCEPTION_NO_CONTROLLER:
				// get params and load distribution
				$params = $errors->request->getParams();
				if ( strcasecmp($params["controller"], "tab_id") == 0 ) {
					// fwd to index
					$this->_forward('index', 'index', 'distribution', array("tab_id" => $params["action"]));
					return;
				} else {
					$distribution = new Petolio_Model_PoContentDistributions();
					$distribution->getMapper()->findOneByField("url", $params["controller"], $distribution);
					if ( $distribution->getId() ) {
						// fwd to index
						$this->_forward('index', 'index', 'distribution', array("url" => $distribution->getUrl()));
						return;
					}
				}
