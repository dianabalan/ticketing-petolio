...
class Petolio_Model_DbTable_Ticket_PoTicketsClients extends Zend_Db_Table_Abstract
{
	...
	public function fetchSpTickets($user_id)
	{
		...
		$query = $db->select()
			->from($this->_name, $decriptiveColumns)
			->join("po_clients",$this->_name.".client_id = po_clients.id","","petolio")
			->join("po_tickets_client_wf",$this->_name.".ID = po_tickets_client_wf.tickets_clients_id",array("po_tickets_client_wf.flgworkflow", "po_tickets_client_wf.date_created"),"petolio")
			->join("po_tickets",$this->_name.".ticket_id = po_tickets.id",array("po_tickets.description","po_tickets.ticket_date"),"petolio")
			->join("po_users","po_clients.client_id = po_users.id",array("po_users.name","po_users.email"),"petolio")
			->where("po_tickets.user_id = :user_id");
