...
class Petolio_Model_PoSubscriptions extends MainModel {
	protected $_Id;
	protected $_UserId;
	protected $_Scope;
	protected $_EntityId;
	...
	public function getMapper() {
		if(is_null($this->_mapper))
			$this->setMapper(new Petolio_Model_PoSubscriptionsMapper());
	...
	public function getSubscribers($where = false, $order = false, $limit = false) {
		...
		$select = $db->select()->setIntegrityCheck(false)
			->from(array('a' => 'po_subscriptions'), array('user_id'))

			// get pet owner
			->joinLeft(array('b' => 'po_users'), "a.user_id = b.id", array('user_name' => 'name', 'user_email' => 'email', 'user_dash_email_notification' => 'dash_email_notification'));
