...
class Petolio_Model_PoShotRecordRightsMapper {

    /**
     * $_dbTable - instance of Petolio_Model_DbTable_PoShotRecordRights
     *
     * @var Petolio_Model_DbTable_PoShotRecordRights
     */
    protected $_dbTable;
     ...
     * @param string $field
     * @param mixed $value
     * @param Petolio_Model_PoShotRecordRights $cls
     */
     ...
     *
     * @param new Petolio_Model_PoShotRecordRights $cls
     * @return array
     *
     */
    ...
    public function findByField($field, $value, $cls)
    {
            ...
            foreach ($rows as $row) {
                    $cls = new Petolio_Model_PoShotRecordRights();
                    $result[]=$cls;
		            $cls->setId($row->id)
						->setShotRecordId($row->shot_record_id)
						->setUserId($row->user_id);
     ...
     *
     * @param Petolio_Model_DbTable_PoShotRecordRights $dbTable
     ...
     *
     * @return Petolio_Model_DbTable_PoShotRecordRights
     */
    public function getDbTable()
    {
        if (null === $this->_dbTable) {
            $this->setDbTable('Petolio_Model_DbTable_PoShotRecordRights');
        }
    ...
    public function save(Petolio_Model_PoShotRecordRights $cls,$ignoreEmptyValuesOnUpdate=true, $escapeValues = false)
    {
       	$data = $cls->toArray();
    ...
    public function find($id, Petolio_Model_PoShotRecordRights $cls)
    {
    ...
    public function fetchAll()
    {
        ...
        foreach ($resultSet as $row) {
            $entry = new Petolio_Model_PoShotRecordRights();
            $entry->setId($row->id)
				  ->setShotRecordId($row->shot_record_id)
				  ->setUserId($row->user_id)
				  			  ->setMapper($this);
    ...
    /**
     * returns an array of PoUsers who have access to this shot record
     *
     * @param int $shot_record_id
     */
    public function findAllowedUsers($shot_record_id = -1) {
        $resultSet = $this->getDbTable()->findAllowedUsers($shot_record_id);
        $entries   = array();
